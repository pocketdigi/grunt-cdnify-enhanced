    ...
    grunt.registerMultiTask('cdnify', 'Converts local URLs to CDN ones.', function () {
        ...
        if (typeof base === 'string') {
            rewriteURL = function (origUrl) {
                //文件所在目录相对路径
                var dirRelativePath=path.dirname(path.relative(currentDestDir,currentDestFile));
                var fileCdnBase=base+dirRelativePath+"/";
                return isLocalPath(origUrl) ? url.resolve(fileCdnBase, origUrl) : origUrl;
            };
